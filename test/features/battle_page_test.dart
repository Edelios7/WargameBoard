...
void main() {
  ...
  testWidgets(
      'the live battle dashboard renders without overflow on a phone-sized screen',
      (tester) async {
  ...
  testWidgets(
      'the unit management dialog shows stats and applies modifiers to them',
      (tester) async {
    final armyId = await database.armyDao.createArmy(
      name: 'Ma liste',
      factionId: seedFactionId,
    ...
    await database.battleDao.startBattle(armyId: armyId, opponentName: 'Marc');

    await tester.pumpWidget(wrap());
    await tester.pumpAndSettle();

    await tester.ensureVisible(find.text('Captain ×1'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Captain ×1'));
    await tester.pumpAndSettle();

    // Caractéristiques de base affichées (M6 T4 Sv3 W5 Ld6 OC1) et les
    // armes de la fiche.
    expect(find.text('4'), findsOneWidget); // Endurance
    expect(find.textContaining('Mêlée'), findsWidgets);

    // Ajouter un bonus d'Endurance (déjà présélectionnée par défaut) le
    // reflète immédiatement dans la caractéristique affichée.
    await tester.ensureVisible(find.text('Ajouter un bonus/malus'));
    await tester.tap(find.text('Ajouter un bonus/malus'));
    await tester.pumpAndSettle();

    expect(find.text('5'), findsWidgets); // 4 + 1
    expect(find.text('4'), findsNothing);
  });

  testWidgets(
      'the unit management dialog renders without overflow on a phone-sized screen',
      (tester) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
