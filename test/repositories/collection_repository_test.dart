...
void main() {
  ...
  test('reducing quantity to match an already-clamped assembled/painted count '
      'does not award the squad-completed XP bonus', () async {
    ...
    final afterProgress = await database.xpDao.getCategoryTotals();
  ...
  test('genuinely assembling the last model to reach the quantity still '
      'awards the squad-completed bonus', () async {
    final results = await database.datasheetDao.search('Captain');
    final entryId = await repository.addEntry(
      datasheetId: results.single.id,
      quantity: 3,
  ...
  test('moving a wishlist item into the collection awards the new-box XP, '
      'just like adding it directly', () async {
    final results = await database.datasheetDao.search('Captain');
  ...
  test('explicitly lowering painted then raising it back does not award XP '
      'twice for the same models', () async {
    final results = await database.datasheetDao.search('Captain');
    final entryId = await repository.addEntry(
      datasheetId: results.single.id,
      quantity: 5,
    ...
    await repository.updateCounts(entryId, assembled: 5, primed: 5, painted: 5);
  ...
  test('deleting a collection entry and re-adding it does not re-award the '
      'new-box XP for the same purchase', () async {
    final results = await database.datasheetDao.search('Captain');
    final entryId = await repository.addEntry(
      datasheetId: results.single.id,
      quantity: 1,
    ...
    await repository.addEntry(datasheetId: results.single.id, quantity: 1);
    final afterRoundTrip = await database.xpDao.getCategoryTotals();
    expect(
  ...
  test("deleting a collection entry removes its personal photo file instead "
      "of leaving it orphaned on disk forever (the entry's id is never "
      'reused)', () async {
    final results = await database.datasheetDao.search('Captain');
    final entryId = await repository.addEntry(
      datasheetId: results.single.id,
    ...
    final entriesPhotoDir = Directory(
      p.join(
    ...
    final photoFile = File(p.join(entriesPhotoDir.path, '$entryId.png'))
      ..writeAsBytesSync([0]);
