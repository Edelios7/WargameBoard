...
void main() {
  ...
  test(
    'creating an army and adding units computes correct total points',
    () async {
  ...
  test('detachment and enhancement points are included in the total', () async {
    final armyId = await database.armyDao.createArmy(
    ...
    final detachments = await database.armyDao.getDetachmentsForFaction(
      seedFactionId,
    );
    expect(detachments.map((d) => d.id), contains(detAngelicHost));

    final options = await database.armyDao.getEnhancementsForDetachment(
      detAngelicHost,
    );
  ...
  test(
    'stratagems are listed for a detachment, ordered by command points',
    () async {
      final stratagems = await database.armyDao.getStratagemsForDetachment(
        detAngelicHost,
      );
  ...
  test('updateNotes sets and clears the army notes', () async {
    ...
    await database.armyDao.updateNotes(
      armyId,
      'Prévoir un plan B contre les chars.',
    );
  ...
  test('deleteArmy also clears battle state tied to its units and unlinks '
      'past battles, instead of leaving orphaned rows', () async {
    final armyId = await database.armyDao.createArmy(
  ...
  test('hasValidationErrors reflects both the points limit and the '
      'enhancement-count rule, in the list summary too', () async {
    final armyId = await database.armyDao.createArmy(
      name: 'Liste avec détachement',
      factionId: seedFactionId,
      detachmentId: detAngelicHost,
  ...
  test('a datasheet with no cost bracket data resolves to an unknown cost, '
    ...
    final army = await database.armyDao.getArmy(armyId);
    final noCostUnit = army!.units.firstWhere(
      (u) => u.datasheetId == noCostDatasheetId,
    );
    ...
    expect(validation.warnings, contains(ArmyValidationIssue.unknownUnitCosts));
  });
  ...
  test('getArmy flags Character datasheets via isCharacter', () async {
    ...
    final army = await database.armyDao.getArmy(armyId);
    final captainUnit = army!.units.firstWhere((u) => u.id == captainUnitId);
  ...
  test(
    ...
    () async {
      ...
      final captainUnit = attached!.units.firstWhere(
        (u) => u.id == captainUnitId,
      );
      expect(captainUnit.attachedToUnitId, squadUnitId);
      expect(captainUnit.attachedToUnitName, 'Death Company Marines');
      expect(attached.leadersAttachedTo(squadUnitId).map((u) => u.id), [
        captainUnitId,
      ]);
      ...
      final detachedCaptain = detached!.units.firstWhere(
        (u) => u.id == captainUnitId,
      );
  ...
  test('removing the host unit detaches any Character that was attached to '
      'it, instead of leaving a dangling reference', () async {
    final armyId = await database.armyDao.createArmy(
      name: 'Escouade test',
      factionId: seedFactionId,
    );
  ...
  test('reorderUnits persists a new display order (drag-and-drop reorder in '
      'the grid)', () async {
    final armyId = await database.armyDao.createArmy(
      name: 'Escouade test',
      factionId: seedFactionId,
    ...
    await database.armyDao.reorderUnits(armyId, [
      squadUnitId,
      guardUnitId,
      captainUnitId,
    ]);
